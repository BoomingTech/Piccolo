...
highp vec2 lut_uv(highp float red, highp float green,
                  highp float blue_slice, highp float slice_size) {
...
highp vec4 get_lut_color(highp vec4 color, highp float slice_size) {
    ...
    highp vec4 color_left = textureLod(color_grading_lut_texture_sampler,
                                       lut_uv(color.r, color.g, slice, slice_size), 0.0);
    highp vec4 color_right = textureLod(color_grading_lut_texture_sampler,
                                        lut_uv(color.r, color.g, slice + 1.0F, slice_size), 0.0);
    return vec4(
