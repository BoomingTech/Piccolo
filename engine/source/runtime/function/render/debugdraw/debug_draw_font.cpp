...
namespace Piccolo
{
    void DebugDrawFont::loadFont()
    {
        ...
        for (unsigned char character = m_range_l; character <= m_range_r; character++)
        {
            ...
            int y          = ascent + c_y1 - 2;
            int byteOffset = roundf(leftSideBearing * scale) + (character - m_range_l) % m_numOfCharacterInOneLine * m_singleCharacterWidth +
                             ((character - m_range_l) / m_numOfCharacterInOneLine * m_singleCharacterHeight + y) * m_bitmap_w;
    ...
    void DebugDrawFont::inialize() { loadFont(); }
    ...
    RHIImageView* DebugDrawFont::getImageView() const { return m_font_imageView; }
} // namespace Piccolo