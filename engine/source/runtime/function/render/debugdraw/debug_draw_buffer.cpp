...
namespace Piccolo
{
    ...
    void   DebugDrawAllocator::cacheUniformObject(Matrix4x4 proj_view_matrix) { m_uniform_buffer_object.proj_view_matrix = proj_view_matrix; }
    ...
    size_t DebugDrawAllocator::getUniformDynamicCacheOffset() const { return m_uniform_buffer_dynamic_object_cache.size(); }
    ...
    const size_t DebugDrawAllocator::getCylinderVertexBufferSize() const { return (m_circle_sample_count * 2) * 5 * 2; }
    const size_t DebugDrawAllocator::getCapsuleVertexBufferSize() const
    {
        return (m_circle_sample_count * 2) * m_circle_sample_count * 4 + (2 * m_circle_sample_count) * 2 +
               (2 * m_circle_sample_count) * m_circle_sample_count * 4;
    }
    const size_t DebugDrawAllocator::getCapsuleVertexBufferUpSize() const { return (m_circle_sample_count * 2) * m_circle_sample_count * 4; }
    const size_t DebugDrawAllocator::getCapsuleVertexBufferMidSize() const { return 2 * m_circle_sample_count * 2; }
    const size_t DebugDrawAllocator::getCapsuleVertexBufferDownSize() const { return 2 * m_circle_sample_count * m_circle_sample_count * 4; }
