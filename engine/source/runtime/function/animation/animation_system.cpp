...
namespace Piccolo
{
    ...
    BlendStateWithClipData AnimationManager::getBlendStateWithClipData(const BlendState& blend_state)
    {
        ...
        for (auto& iter : blend_state.m_blend_mask_file_path)
        {
            ...
            tryLoadAnimationSkeletonMap(m_skeleton_mask_cache[iter]->m_skeleton_file_path);
        }
        size_t skeleton_bone_count = m_skeleton_definition_cache[blend_masks[0]->m_skeleton_file_path]->m_bones_map.size();
