...
namespace Piccolo
{
    ...
    {
        m_is_flat = skeleton_definition.m_is_flat;
        if (m_bones != nullptr)
        {
            delete[] m_bones;
        }
        if (!m_is_flat || !skeleton_definition.m_in_topological_order)
        {
            // LOG_ERROR
            return;
        }
        m_bone_count = skeleton_definition.m_bones_map.size();
        m_bones      = new Bone[m_bone_count];
        for (size_t i = 0; i < m_bone_count; i++)
        {
            const RawBone bone_definition = skeleton_definition.m_bones_map[i];
            Bone*         parent_bone     = find_by_index(m_bones, bone_definition.m_parent_index, i, m_is_flat);
    ...
    void Skeleton::applyAnimation(const BlendStateWithClipData& blend_state)
    {
        ...
        for (size_t clip_index = 0; clip_index < 1; clip_index++)
        {
            ...
            for (size_t node_index = 0; node_index < animation_clip.m_node_count && node_index < anim_skel_map.m_convert.size(); node_index++)
            {
                ...
                Quaternion rotation =
                    Quaternion::nLerp(lerp_ratio, channel.m_rotation_keys[current_frame_low], channel.m_rotation_keys[current_frame_high], true);
